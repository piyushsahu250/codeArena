...
// Attempted fix, reverted: rendering an entry's lines as one paragraph with manual `break`s
// between TextRuns, hoping mammoth would extract each break as "\n". Confirmed live it does not
// (even with mammoth's `{ allowLineBreaks: true }` option) — every line came back concatenated
// with NO separator at all ("TitleDescriptionTech: ...https://..."), which is a strictly worse
// outcome than the original per-paragraph approach. See resumeParser.js's `mergeMetadataOnlyBlocks`
// for the fix that actually shipped: it re-attaches an isolated tech/role/date/URL line back onto
// the entry before it on the PARSING side instead, which doesn't depend on this generator-side
// (and apparently unreliable) line-break behavior at all.
