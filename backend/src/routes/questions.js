...
// normalizeOptions/normalizeCorrectIndices moved to utils/mcqOptions.js (2026-09) so they're a
// pure, dependency-free module regression tests can require directly — see that file's own header
// comment and backend/test/mcqOptions.test.js. Nothing about their behavior changed in the move.
...
router.post("/preview-starter-code", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), requireFeature("question_bank"), (req, res) => {
  try {
...
router.get("/analytics/summary", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
...
router.post("/", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
    const {
...
router.get("/", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.get("/meta/filters", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.get("/folders", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
...
router.post("/folders", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.patch("/folders/:id", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.get("/folders/:id/delete-preview", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  const folder = await prisma.questionFolder.findUnique({
    where: { id: req.params.id },
    include: { shares: { select: { staffId: true } } },
  });
...
router.delete("/folders/:id", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  const folder = await prisma.questionFolder.findUnique({
    where: { id: req.params.id },
    include: { shares: { select: { staffId: true } } },
  });
...
router.get("/folders/:id/shares", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.post("/folders/:id/shares", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
...
router.delete("/folders/:id/shares/:staffId", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
    const folder = await prisma.questionFolder.findUnique({ where: { id: req.params.id }, select: { id: true, name: true, createdById: true, instituteId: true } });
    if (!folder) return res.status(404).json({ error: "Question bank not found" });
    if (req.requesterInstituteId && folder.instituteId && folder.instituteId !== req.requesterInstituteId) {
...
router.post("/folders/:id/merge", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.post("/bulk-move", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.post("/bulk-assign-subject", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  const questionIds = Array.isArray(req.body.questionIds) ? req.body.questionIds : [];
  if (questionIds.length === 0) return res.status(400).json({ error: "No questions selected" });
...
router.post("/bulk-status", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  const questionIds = Array.isArray(req.body.questionIds) ? req.body.questionIds : [];
...
router.post("/bulk-delete", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  const questionIds = Array.isArray(req.body.questionIds) ? req.body.questionIds : [];
  if (questionIds.length === 0) return res.status(400).json({ error: "No questions selected" });
  const owned = await prisma.question.findMany({ where: { id: { in: questionIds } } });
...
router.post("/bulk-copy", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  const questionIds = Array.isArray(req.body.questionIds) ? req.body.questionIds : [];
  const folderId = req.body.folderId || null;
  if (questionIds.length === 0) return res.status(400).json({ error: "No questions selected" });
...
// is left untouched.
router.post("/folders/:id/clear", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.get("/export", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.post("/:id/duplicate", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
...
router.get("/:id", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
...
router.patch("/:id", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
...
router.delete("/:id", authenticate, requireRole("ADMIN", "SUPER_ADMIN", "INSTITUTE_ADMIN", "STAFF"), attachRequesterInstitute, requireFeature("question_bank"), async (req, res) => {
  try {
    const existing = await prisma.question.findUnique({
      where: { id: req.params.id },
      include: { folder: { include: { shares: { select: { staffId: true } } } } },
