...
router.post("/:id/bulk-import", authenticate, requireRole("ADMIN"), attachRequesterInstitute, upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    const added = [], alreadyExisting = [], invalidInstitute = [], invalidRollNumber = [], failed = [];
    const toCreate = [];
    const seen = new Set();
    ...
    await logAudit({
      ...
      details: {
        ...
        invalidInstituteCount: invalidInstitute.length, invalidRollNumberCount: invalidRollNumber.length, failedCount: failed.length,
      },
    });
    ...
    res.json({
      ...
      invalidRollNumberCount: invalidRollNumber.length,
      failedCount: failed.length,
      added, alreadyExisting, invalidInstitute, invalidRollNumber, failed,
    });
  } catch (err) {
    console.error(err);
